...
def mode(role: str) -> str:
    if os.getenv('JWT_PRIVATE_JWK') or os.getenv('JWT_PRIVATE_JWK_PATH') or os.getenv('JWT_PRIVATE_JWK_NAME'): return 'EdDSA'
    if os.getenv('JWT_PUBLIC_JWK') or os.getenv('JWT_PUBLIC_JWK_NAME') or os.getenv('JWT_JWKS_URL') or os.getenv('JWT_JWKS_URL_NAME'): return 'EdDSA'
    return 'HS512'
...
def get_hs_secret_bytes() -> bytes:
    ...
    if not s: raise RuntimeError("JWT secret missing: set JWT_SECRET_NAME -> bound secret, or JWT_SECRET")
    try:
